...
contract EmblemLPStaking is Ownable {
    ...
    function add(uint256 _emblemPerBlock, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingEmblemReward(uint256 _poolId, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEmblemPerShare = accEmblemPerShare.add(emblemReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _poolId) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 emblemReward = multiplier.mul(pool.perBlockEmblemAllocated);
        pool.accEmblemPerShare = pool.accEmblemPerShare.add(emblemReward.mul(1e12).div(lpSupply));

...
contract EmblemTokenStaking is Ownable {
    ...
    IERC20 public emblemTokenContract;
    ...
    constructor(
        IERC20 _emblemContractAddress,
        uint256 _emblemPerBlock,
        uint256 _startBlock
    ) public {
        emblemTokenContract = _emblemContractAddress;
    ...
    function add(uint256 _emblemPerBlock, IERC20 _stakingToken, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function fund(uint256 _amount) public {
        ...
        require(emblemTokenContract.balanceOf(_from) >= _amount, 'fund: expected an address that contains enough Emblem for Transfer');
        emblemTokenContract.transferFrom(_from, address(this), _amount);
    ...
    function pendingEmblemReward(uint256 _poolId, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && tokenSupply != 0) {
            ...
            accEmblemPerShare = accEmblemPerShare.add(emblemReward.mul(1e12).div(tokenSupply));
        }
    ...
    function harvest(uint256 _poolId) external returns (uint256) {
        ...
        if (user.amount > 0) {
            ...
            safeEmblemTransfer(msg.sender, pending);
            return pending;
        }
        return 0;
    }

    function getLockedEmblemView() external view returns (uint256) {
        return emblemTokenContract.balanceOf(address(this));
    }
    ...
    function updatePool(uint256 _poolId) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 emblemReward = multiplier.mul(pool.perBlockEmblemAllocated);
        pool.accEmblemPerShare = pool.accEmblemPerShare.add(emblemReward.mul(1e12).div(tokenSupply));

        pool.lastRewardBlock = block.number;
    }

    // Deposit staking tokens to Contract for RIO allocation.
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            safeEmblemTransfer(address(msg.sender), pending);
        }
        if(_amount > 0) {
    ...
    function safeEmblemTransfer(address _to, uint256 _amount) internal {
        address _from = address(this);
        uint256 emblemBal = emblemTokenContract.balanceOf(_from);
        if (_amount > emblemBal) {
            emblemTokenContract.transfer(_to, emblemBal);
        } else {
            emblemTokenContract.transfer(_to, _amount);
        }
    }
}
